...
#ifndef __H__UG__CONVECTION_DIFFUSION__UPWIND_INTERFACE__
#define __H__UG__CONVECTION_DIFFUSION__UPWIND_INTERFACE__
...
template <int dim>
class IConvectionDiffusionUpwind
{
    protected:
    ///    used traits
    ...
    /// Abbreviation for own type
        typedef IConvectionDiffusionUpwind<dim> this_type;
    ...
    /// returns the upwind velocity
        number upwind_value(const size_t scvf,
                                   const LocalVector& CornerVel,
                                   const number vStdVel[]) const;
    /// returns the upwind velocity
        number downwind_value(const size_t scvf,
                                   const LocalVector& CornerVel,
                                   const number vStdVel[]) const;
...
template <typename TFVGeom, typename TAssFunc>
void
IConvectionDiffusionUpwind<dim>::
...
//    add corner shapes
    for(size_t sh = 0; sh < num_sh(); ++sh)
        value += upwind_shape_sh(scvf, sh) * CornerVel(0, sh);
...
//    compute ip vel
    for(size_t scvf2 = 0; scvf2 < num_scvf(); ++scvf2)
        value+=upwind_shape_ip(scvf, scvf2)*vStdValue[scvf2];
...
number
IConvectionDiffusionUpwind<dim>::
downwind_value(const size_t scvf,
           const LocalVector& CornerVel,
           const number vStdValue[]) const
{
//    reset result
...
//    add corner shapes
    for(size_t sh = 0; sh < num_sh(); ++sh)
        value += downwind_shape_sh(scvf, sh) * CornerVel(0, sh);
...
//    compute ip vel
    for(size_t scvf2 = 0; scvf2 < num_scvf(); ++scvf2)
        value+=downwind_shape_ip(scvf, scvf2)*vStdValue[scvf2];
...
class ConvectionDiffusionUpwindRegister
{
    public:
    ///    Base class
    ...
    public:
    ///    constructor
        ConvectionDiffusionUpwindRegister() {register_func(Int2Type<dim>());}
...
#endif /* __H__UG__ConvectionDiffusion__UPWIND_INTERFACE__ */
